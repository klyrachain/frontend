...
type Ctx = { params: Promise<{ claimLinkId: string }> };
...
export async function GET(_req: Request, ctx: Ctx) {
  ...
  const { claimLinkId } = await ctx.params;
  const enc = encodeURIComponent(claimLinkId.trim());
  try {
    const res = await fetch(`${backend}/api/klyra/claims/by-link/${enc}`, {
      cache: "no-store",
